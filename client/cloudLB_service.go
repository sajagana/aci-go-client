...
func (sm *ServiceManager) CreateCloudL4L7LB(name string, tenant string, description string, cloudLBAttr models.CloudL4L7LBAttributes) (*models.CloudL4L7LB, error) {
	rn := fmt.Sprintf(models.RnCloudLB, name)
	parentDn := fmt.Sprintf(models.ParentDnCloudLB, tenant)
	cloudLB := models.NewCloudL4L7LB(rn, parentDn, description, cloudLBAttr)
	err := sm.Save(cloudLB)
	return cloudLB, err
}

func (sm *ServiceManager) ReadCloudL4L7LB(name string, tenant string) (*models.CloudL4L7LB, error) {
	rn := fmt.Sprintf(models.RnCloudLB, name)
	parentDn := fmt.Sprintf(models.ParentDnCloudLB, tenant)
	...
	cloudLB := models.CloudL4L7LBFromContainer(cont)
	return cloudLB, nil
}

func (sm *ServiceManager) DeleteCloudL4L7LB(name string, tenant string) error {
	rn := fmt.Sprintf(models.RnCloudLB, name)
	parentDn := fmt.Sprintf(models.ParentDnCloudLB, tenant)
	dn := fmt.Sprintf("%s/%s", parentDn, rn)
	return sm.DeleteByDn(dn, models.CloudLBClassName)
}

func (sm *ServiceManager) UpdateCloudL4L7LB(name string, tenant string, description string, cloudLBAttr models.CloudL4L7LBAttributes) (*models.CloudL4L7LB, error) {
	rn := fmt.Sprintf(models.RnCloudLB, name)
	parentDn := fmt.Sprintf(models.ParentDnCloudLB, tenant)
	cloudLB := models.NewCloudL4L7LB(rn, parentDn, description, cloudLBAttr)
...
func (sm *ServiceManager) ListCloudL4L7LB(tenant string) ([]*models.CloudL4L7LB, error) {
	parentDn := fmt.Sprintf(models.ParentDnCloudLB, tenant)
	dnUrl := fmt.Sprintf("%s/%s/%s.json", models.BaseurlStr, parentDn, models.CloudLBClassName)
	cont, err := sm.GetViaURL(dnUrl)
	list := models.CloudL4L7LBListFromContainer(cont)
	return list, err
}
